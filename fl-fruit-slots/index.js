  ...
  methods: {
    ...
    */
    ...
    /**
      ...
      未找到返回-1
    */
    findPrizeIndex: function findPrizeIndex(prizeIndex) {
      var prizeList = this.props.prizeList;
      var order = this.data.activeOrder;
      for (var i = 0; i < this.prizeLength; i++) {
        if (i==order.indexOf(prizeIndex)) {
          this.data.prizeItem = prizeList[order[i]];
          return i;
        }
      }
      return -1;
    ...
    start: function start() {
      ...
      this.props.onFinish(activeIndex, this.data.prizeItem);
    }
  }
});